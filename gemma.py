...
class GemmaTranslationModel:
    ...
    def create_translation_prompt(self, text: str, source_lang: str, target_lang: str,
        ...
        if few_shot_examples:
            ...
            for example in few_shot_examples:
                if (example['source_lang'] == source_lang and 
    ...
    def translate(self, text: str, source_lang: str, target_lang: str,
        ...
        return self._translate_direct(text, source_lang, target_lang, few_shot_examples, max_length, temperature)

    def _translate_direct(self, text: str, source_lang: str, target_lang: str,
                         few_shot_examples: Optional[List[Dict]] = None,
                         max_length: int = 128, temperature: float = 0.7) -> str:
        ...
        try:
            ...
            with torch.no_grad():
                outputs = self.model.generate(
                    **inputs,
                    max_new_tokens=min(max_length, 20),  # Very short to force single word/phrase
                    do_sample=False,
                    temperature=0.0,
            ...
            for prefix in prefixes_to_remove:
                if translation.lower().startswith(prefix.lower()):
                    ...
                    if colon_pos > 0 and quote_pos > 0:
                        translation = translation[min(colon_pos, quote_pos)+1:].strip()
                    elif colon_pos > 0:
            ...
            translation = translation.split('**')[1] if '**' in translation else translation
            translation = translation.split('*')[1] if '*' in translation and not translation.startswith('*') else translation
            translation = translation.strip('"\'')  # Remove quotes
            translation = translation.split('(')[0].strip()  # Remove parenthetical explanations
            translation = translation.split('"')[0].strip()  # Remove any remaining quotes
    ...
    def _translate_chat(self, text: str, source_lang: str, target_lang: str,
                       few_shot_examples: Optional[List[Dict]] = None,
                       max_length: int = 128, temperature: float = 0.7) -> str:
    ...
    def batch_translate(self, texts: List[str], source_lang: str, target_lang: str,
                       few_shot_examples: Optional[List[Dict]] = None) -> List[str]:
        """Translate multiple texts in batch"""
        translations = []
        
        for text in texts:
            try:
                translation = self.translate(text, source_lang, target_lang, few_shot_examples)
    ...
    def evaluate_translation_quality(self, source_texts: List[str], 
        ...
        try:
            ...
            return {
                ...
                'success_rate': sum(1 for t in translations if t.strip()) / len(translations)
            }
...
def main():
    ...
    few_shot_translation = model.translate(test_text, source_lang, target_lang, few_shot_examples)
