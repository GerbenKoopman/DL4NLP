...
def plot_training_history(
    csv_path: Path,
    out_dir: Path,
    train_task_filter: list | None = None,
...
def plot_evaluation_summary(
    csv_path: Path,
    out_dir: Path,
    task_filter: list | None = None,
    eval_type_filter: list | None = None,
) -> None:
    df = pd.read_csv(csv_path)
    model = df['model'].iloc[0] if 'model' in df.columns and not df.empty else 'unknown'

    # Pivot by evaluation_type x task_type
    pivot = df.pivot_table(index='task_type', columns='evaluation_type', values='score', aggfunc='mean')
    # Optional filtering
    if task_filter:
...
def summarize_comparison(json_path: Path, out_dir: Path) -> None:
    ...
    bar_height = 0.38
    offset = bar_height / 2
    plt.barh([i + offset for i in y], df['few_shot_improvement'], height=bar_height, color='#2a9d8f', label='Few-shot (5) Improvement')
    plt.barh([i - offset for i in y], df['zero_shot_improvement'], height=bar_height, color='#e76f51', label='Zero-shot Improvement')
...
def main() -> None:
    ...
    parser.add_argument('--train_tasks', default='all', help='Comma-separated training task series to include (e.g., be_en,en_be,meta_average) or "all"')
    parser.add_argument('--no_meta_average', action='store_true', help='Exclude meta_average from training history plots')
    parser.add_argument('--tasks', default='all', help='Comma-separated task_types to include (e.g., az_tr,be_uk) or "all"')
    parser.add_argument('--eval_types', default='all', help='Comma-separated evaluation types to include (e.g., zero_shot,few_shot_5,transfer_5) or "all"')
    ...
    train_task_filter = None if args.train_tasks.strip().lower() == 'all' else [t.strip() for t in args.train_tasks.split(',') if t.strip()]
    ...
    for csv_path in train_csvs:
        plot_training_history(csv_path, plots_dir, train_task_filter=train_task_filter, include_meta_average=include_meta_average)

    # Build filters
    task_filter = None if args.tasks.strip().lower() == 'all' else [t.strip() for t in args.tasks.split(',') if t.strip()]
    eval_type_filter = None if args.eval_types.strip().lower() == 'all' else [e.strip() for e in args.eval_types.split(',') if e.strip()]
    ...
    for csv_path in eval_csvs:
        plot_evaluation_summary(csv_path, plots_dir, task_filter=task_filter, eval_type_filter=eval_type_filter)
