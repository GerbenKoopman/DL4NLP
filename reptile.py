...
class ReptileConfig:
    ...
    base_langs: List[str] = None   # Azerbaijani, Belarusian, English
    target_langs: List[str] = None # Turkish, Ukrainian
...
class ReptileMetaLearner:
    ...
    def __init__(self, config: ReptileConfig):
        self.config = config
        self.gemma_model = get_cached_gemma_model(config.gemma_model, config.device)
    ...
    def create_task_episodes(self, tasks: List[Dict], task_type: str) -> List[Tuple[List[Dict], List[Dict]]]:
        ...
        if len(task_examples) < self.config.support_size + self.config.query_size:
            logger.warning(f"Insufficient examples for {task_type}: {len(task_examples)}")
            return []
        ...
        for _ in range(3):  # Create 3 episodes per task type
            ...
            query_set = task_examples[self.config.support_size:self.config.support_size + self.config.query_size]
    ...
    def evaluate_query_set(self, query_examples: List[Dict], 
                          adaptation_context: List[Dict]) -> float:
    ...
    def reptile_step(self, tasks: List[Dict], task_types: List[str]) -> Dict[str, float]:
        ...
        sampled_tasks = np.random.choice(task_types, 
                                       min(len(task_types), 4), 
                                       replace=False)
    ...
    def evaluate_transfer(self, test_tasks: List[Dict], num_shots: int = 5) -> Dict[str, float]:
        ...
        for task_type in unique_target_tasks:
            task_examples = [task for task in test_tasks if task['task_type'] == task_type]
            ...
            query_examples = task_examples[num_shots:num_shots + 5]  # Test on 5 examples
